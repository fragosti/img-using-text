...
export const fileToText = (file, text, width, stretch, options) => {
	return fileToPixels(file, width, stretch).then((imgPixels) => {
		return pixelsToText(imgPixels, text, options)
	})
}

export const urlToText = (url, text, width, stretch, options) => {
	return urlToPixels(url, width, stretch).then((imgPixels) => {
		return pixelsToText(imgPixels, text, options)
	})
}
